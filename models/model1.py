...
def save_response(emotion):
    ...
    user_response = {
        ...
        "predicted_emotion": predict_emotion(
            prediction_text, model, vectorizer, emotion_map
        ),
...
emotion_label = tk.Label(
    root, text="Choose the correct emotion based on your input:", font=label_font
)
...
for i, emotion in enumerate(emotions):
    button = tk.Button(
        root, text=emotion, command=lambda e=emotion: save_response(e), font=button_font
    )
...
correct_predictions = sum(
    ...
    if response["predicted_emotion"] == response["user_emotion"]
)
accuracy_percentage = (
...
accuracy_message = (
    f"The bot has been {accuracy_percentage:.2f}% accurate based on your responses."
)
...
with open("../data/newdata.csv", "a") as file:
    for response in user_responses:
        ...
        if is_valid_input(input_text):
            mapped_user_emotion = next(
                key for key, value in emotion_map.items() if value == user_emotion
            )
...
report = classification_report(
    y_test, y_pred, target_names=list(emotion_map.values()), output_dict=True
)
...
coef_df = pd.DataFrame(
    coefficients.T, index=feature_names, columns=emotion_map.values()
)


def plot_top_features(class_name, top_n=10):
    class_coefficients = coef_df[class_name]
    top_positive_coefficients = class_coefficients.sort_values(ascending=False).head(
        top_n
    )
    top_negative_coefficients = class_coefficients.sort_values(ascending=False).tail(
        top_n
    )
    ...
    plt.title(
        f"Top {top_n} Positive and Negative Features for {class_name.capitalize()}"
    )
